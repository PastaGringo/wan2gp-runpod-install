...
def deploy(args: argparse.Namespace) -> None:
    ...
    print(f"  Gradio (later): https://{pod_id}-7860.proxy.runpod.net")
    print("─" * 60)
    print()
    print("Next steps in the pod's web terminal (Jupyter → New → Terminal):")
    print()
    print(f"  curl -fsSL {INSTALL_SCRIPT_URL} | bash")
    print()
    print("Then launch Wan2GP:")
    print()
    print("  cd /workspace/Wan2GP && source venv/bin/activate \\")
    print("    && python wgp.py --listen --server-port 7860 --share")
    print()
    print(f"Stop the pod when you're done:  uv run deploy-pod.py stop {pod_id}")
